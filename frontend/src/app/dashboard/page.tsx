...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (user) getMachines().then(setMachines).catch(console.error);
  }, [user]);
  ...
  const startNewConversation = async () => {
    try {
      ...
      setConversations((prev) => [...prev, { id: conv.id, label }]);
            ...
            {conversations.map((c) => (
              <button key={c.id} onClick={() => setActiveConvId(c.id)}
                className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors
                  ${activeConvId === c.id ? "bg-indigo-50 text-indigo-700 font-medium" : "text-slate-600 hover:bg-slate-50"}`}>
                {c.label}
              </button>
            ))}
            {conversations.length === 0 && (
              <p className="text-xs text-slate-400 px-1">No sessions yet. Start a new one.</p>
            )}
          </div>
        </div>
            ...
            <div className="flex gap-1">
              {user?.role === "admin" && (
                <button onClick={() => router.push("/admin")}
                  className="text-xs px-2 py-1 text-slate-500 hover:text-indigo-600 rounded transition-colors">
                  Admin
                </button>
              )}
              <button onClick={logout}
                className="text-xs px-2 py-1 text-slate-400 hover:text-red-500 rounded transition-colors">
                Out
              </button>
            </div>
          </div>
        ...
        <header className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400">
              {selectedMachine ? `Machine: ${selectedMachine.name}` : "No machine filter active"}
            </p>
          </div>
        </header>
        <ChatInterface
